...
function genobj() {
    ...
    var d = {
        ...
        'location': randopeep.address.geo().map(parseFloat)
    };
    o.push(d);
...
setInterval(function() {
    let cars = JSON.parse(fs.readFileSync('./index.get.json', 'utf8'));
    cars.forEach(car => {
        car.location = car.location.map(c => c + (Math.random() < 0.5 ? -1 : 1) * Math.random() * 0.1);
    })
    fs.writeFileSync("./index.get.json", JSON.stringify(cars));
}, 5000);
